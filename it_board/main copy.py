...
class MatrixRain:
    ...
    def render(self, stdscr, max_y: int, max_x: int, intensity: float = 0.3):
        """Render matrix rain with given intensity."""
        for col in self.columns:
                ...
                if 0 <= y < max_y - 1:
                    try:
                        ...
                        if i == 0:
                            ...
                            stdscr.addstr(y, x, char, curses.color_pair(20) | curses.A_BOLD)
                        elif i < 3:
                            stdscr.addstr(y, x, char, curses.color_pair(21) | curses.A_BOLD)
                        else:
                            stdscr.addstr(y, x, char, curses.color_pair(22) | curses.A_DIM)
                    except curses.error:
                        pass
...
class DataUnit2Banner:
    ...
    def _init_colors(self):
        ...
        curses.init_pair(20, curses.COLOR_WHITE, curses.COLOR_BLACK)   # Head
        curses.init_pair(21, curses.COLOR_GREEN, curses.COLOR_BLACK)   # Bright
        curses.init_pair(22, curses.COLOR_GREEN, curses.COLOR_BLACK)   # Dim
        ...
        while True:
            try:
                ...
                if (height, width) != self.last_size:
                    ...
                    if self.matrix_rain:
                        self.matrix_rain.resize(width, height)
                    else:
                        self.matrix_rain = MatrixRain(width, height)
                    stdscr.clear()
                ...
                # Render matrix rain background (subtle)
                if self.matrix_rain:
                    self.matrix_rain.update()
                    self.matrix_rain.render(stdscr, height, width, intensity=0.15)
